...
<div>
  <h1>Penggunaan Emit</h1>

      <Title title="Emit" />
      <p>{{ childMsg }}</p>
      <EmitCom @response="ubahTeks" />
    </div>
    <h1>penggunaan Slot</h1>
    <SlotC>perubahan slot</SlotC>
...
import SlotC from './components/SlotCnya.vue'
import FooterMe from './components/Footer.vue'



  
  const childMsg = ref('Tidak ada data')
  
  function ubahTeks(msg) {
    childMsg.value = msg
    alert(`Pesan yang diterima dari child: ${msg}`)
  }
</script>
